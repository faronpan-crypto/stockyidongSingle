...
def sina_symbol(code):
    """转新浪格式: sh.600001 或 sz.000001"""
    if code.startswith(('6','9','1')): return f"sh{code}"
    if code.startswith(('0','3','2')): return f"sz{code}"
    return f"sh{code}"

# ======== 新浪数据获取 ========
def fetch_sina_daily(symbol, days=60):
    """从新浪获取日K线数据"""
    ...
    try:
        ...
        for i in range(1, len(data)):
            prev_c = float(data[i-1]['close'])
            curr_c = float(data[i]['close'])
            pcts.append(round((curr_c - prev_c) / prev_c * 100, 2))
...
def save_kelly(stock_name, b, p, kelly_r, sug, notes):
    try:
        ...
        cur.execute("INSERT INTO kelly_records (stock_name, calc_time, odds, win_prob, kelly_ratio, suggestion, notes) VALUES (?,?,?,?,?,?,?)",
...
def calc_mean_rev(closes, window=20):
    ...
    az = abs(z)
    prob_map = [(3,95),(2,85),(1.5,75),(1,65),(0.5,55)]
    r['prob'] = 50
    for thresh, pv in prob_map:
        if az >= thresh: r['prob'] = pv; break
    bp = min(az*0.15, 0.5)
...
def main():
    ...
    for idx, (name, code) in enumerate(HOLDINGS, 1):
        ...
        print(f"\n[{idx}/{len(HOLDINGS)}] {name}({code}) sym={sym}", end="")
        dates, closes, pcts = fetch_sina_daily(sym)
        if len(closes) < 20:
            print(f"  ⚠️ 数据不足{len(closes)}条")
            continue
        print(f"  {len(closes)}条数据")
        
        lp = closes[-1]; lpc = pcts[-1] if pcts else 0; ld = dates[-1] if dates else ""
        ...
        if mr['success']:
            print(f"  均值回归: Z={mr['z_score']:.2f} 均价{mr['mean_price']:.2f} σ={mr['std']:.2f} → {mr['type']} ({mr['prob']:.0f}%)")
    ...
    lines.append(f"**生成时间：** {today} | **数据源：** 新浪财经")
    lines.append(f"**股票：** {len(valid)}只 | **算法：** 凯利公式 f=(b·p-q)/b + 均值回归Z-score(20日)")
    lines.append("")
    ...
    for r in sorted(valid, key=lambda x: x['kelly'], reverse=True):
        m = r['ms']
        f = "鱼身" if m.get('fish_body') else ("🐟尾" if m.get('fish_tail') else "⚪")
        lines.append(
    ...
    lines.append("")
    lines.append("| 股票 | 当前价 | MA20均价 | Z-score | 标准差 | 回归概率 | 建议 |")
    lines.append("|------|--------|----------|---------|--------|---------|------|")
    ...
    lines.append("\n---")
    lines.append(f"\n*生成：{today} | 新浪财经 | 凯利公式+均值回归Z-score*")
