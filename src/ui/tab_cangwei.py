    ...
    def _create_holding_tab(self, notebook, tab_name, group_index):
        ...
        for row in range(rows):
            ...
            for col in range(cols):
                ...
                # === 单击/双击判定: 只用 <Button-1>, 用 event.time 时间戳 ===
        ...
        try:
            ...
            for row in range(16):  # 16行
                ...
                for col in range(5):  # 每行5个
                    ...
                    # === 单击/双击判定: 只用 <Button-1>, 用 event.time 时间戳 ===
                    _click_state2 = {'last_time': 0, 'after_id': None}
                    def _open_daily_kline2(idx, gidx):
                        try:
                            hstocks2, _, _, _ = self._get_holding_group_data(gidx)
                            if idx >= len(hstocks2) or not hstocks2[idx]:
                                return
                            sname2, scode2 = hstocks2[idx]
                            if not scode2:
                                scode2 = get_stock_code_by_name(sname2)
                            if not scode2:
                                return
                            def _kwork2():
                                try:
                                    kd2 = self._get_daily_kline_data_tushare(str(scode2).zfill(6), days=120)
                                except Exception:
                                    kd2 = None
                                if kd2:
                                    self.root.after(0, lambda: self._show_daily_kline_zoom(kd2, sname2))
                                else:
                                    self.root.after(0, lambda: messagebox.showwarning("提示",
                                        f"无法获取 {sname2}({scode2}) 的K线数据\n请检查 Tushare 连接或稍后再试", parent=self.root))
                            threading.Thread(target=_kwork2, daemon=True).start()
                        except Exception:
                            import traceback; traceback.print_exc()
                    def _handle_click2(event, idx=i, gidx=group_index):
                        now2 = event.time
                        elapsed2 = now2 - _click_state2['last_time']
                        _click_state2['last_time'] = now2
                        if elapsed2 < 350 and _click_state2['after_id'] is not None:
                            self.root.after_cancel(_click_state2['after_id'])
                            _click_state2['after_id'] = None
                            _open_daily_kline2(idx, gidx)
                            return "break"
                        if _click_state2['after_id'] is not None:
                            self.root.after_cancel(_click_state2['after_id'])
                        _click_state2['after_id'] = self.root.after(350, lambda: self._show_holding_detail(idx, gidx))
                    label.bind("<Button-1>", _handle_click2)
