...
class CrashRallyCalendarApp:
    ...
    def _build_tab_b(self, parent):
        ...
        tk.Button(top, text="下月▶", command=self._shift_month_b(1), font=FONT_SMALL).pack(side=tk.LEFT)
        ...
        self.cal_frame_b.grid(row=1, column=0, sticky="nsew", padx=6, pady=(0, 2))
        # 详情面板 (下半部分, 与日历各占 50%)
        self.detail_b = tk.Text(parent, font=FONT_MAIN, wrap=tk.WORD, bg="#0D0D1F", fg=COLOR_FG)
        self.detail_b.grid(row=2, column=0, sticky="nsew", padx=6, pady=(2, 6))
        # 权重配置
